		...
		_onDrop: function (evt/**Event*/){
			...
			if( evt ){
				...
				if( dragEl ){
					...
					if( !rootEl.contains(placeHolder) ){
						// Remove event
						rootEl.dispatchEvent(_createEvent('remove', {element: dragEl}));

						// Add event
						placeHolder.parentNode.dispatchEvent(_createEvent('add', {index: _index(placeHolder), element: dragEl}));
					}
					else if( placeHolder.nextSibling !== nextEl ){
						// Update event
						rootEl.dispatchEvent(_createEvent('update', {index: _index(placeHolder), element: dragEl}));
					}
				}
