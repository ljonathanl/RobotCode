/**!
 * Sortable modified version for Vue 
...
(function (factory){
	...
	var
		...
		, _createEvent = function (event/**String*/, data){
	...
	function Sortable(el, options){
		...
		options.draggable = options.draggable || "[draggable]"; //options.draggable || el.children[0] && el.children[0].nodeName || (/[uo]l/i.test(el.nodeName) ? 'li' : '*');
	...
	Sortable.prototype = {
		...
		_applyEffects: function (){
			...
			//_css(dragEl, 'opacity', '0.5');
			_css(dragEl, 'display', 'none');
		},
		...
		_onDragStart: function (evt/**Event*/, isTouch/**Boolean*/){
			...
			//_css(placeHolder, 'display', 'block');
			_css(placeHolder, 'opacity', '0.3');
		...
		_onDragOver: function (evt/**Event*/){
			...
			if( !_silent && (activeGroup === this.options.group) && (evt.rootEl === void 0 || evt.rootEl === this.el) ){
				...
				if( el.children.length === 0 || el.children[0] === ghostEl || (el === evt.target) && _ghostInBottom(el, evt) ){
				...
				else if( target && target !== placeHolder && (target.parentNode[expando] !== void 0) ){
					if( lastEl !== target ){
						lastEl = target;
						lastCSS = _css(target);
					...
					var
						...
						, isWide = (target.offsetWidth > placeHolder.offsetWidth)
						, isLong = (target.offsetHeight > placeHolder.offsetHeight)
					...
					if( floating ){
						after = (target.previousElementSibling === placeHolder) && !isWide || (skew > .5) && isWide
					} else {
						after = (target.nextElementSibling !== placeHolder) && !isLong || (skew > .5) && isLong;
					}

					if( after && !nextSibling ){
						el.appendChild(placeHolder);
					} else {
						target.parentNode.insertBefore(placeHolder, after ? nextSibling : target);
					}
				}
			}
		},
		...
		_onDrop: function (evt/**Event*/){
			...
			if( evt ){
				...
				if( dragEl ){
					...
					if( rootEl != placeHolder.parentNode ){
						// Remove event
						rootEl.dispatchEvent(_createEvent('remove', {container: rootEl, element: dragEl}));

						// Add event
						placeHolder.parentNode.dispatchEvent(_createEvent('add', {container: placeHolder.parentNode, element: dragEl, index: _index(placeHolder)}));
					}
					else if( placeHolder.nextSibling !== nextEl ){
						// Update event
						rootEl.dispatchEvent(_createEvent('update', {container: rootEl, element: dragEl, index: _index(placeHolder)}));
					}
				}

				//document.body.appendChild(placeHolder);
				//_css(placeHolder, 'display', 'none');
